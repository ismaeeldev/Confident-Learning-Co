...
const REASON_COPY: Record<string, { heading: string; body: string; ctaLabel: string; ctaHref: string }> = {
  signed_out: {
  ...
  not_active_member: {
    ...
    ctaLabel: "Continue your membership",
    ctaHref: PUBLIC_ROUTES.checkoutMembershipJoin,
  },
};
...
export default async function AccessPendingPage({ searchParams }: AccessPendingPageProps) {
  ...
  const copy = REASON_COPY[reason ?? ""] ?? REASON_COPY.signed_out;
